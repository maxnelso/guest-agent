...
import (
	...
	pb "github.com/GoogleCloudPlatform/guest-agent/google_guest_agent/protos"
...
func runScript(path string, disks string, timeout int) (int, error) {
	...
	select {
	case <-timer:
		logger.Errorf("timeout while running script at %s", path)
	...
	case err := <-done:
		if err != nil {
			...
			logger.Errorf("error while running script at %s, return with exit code %d", path, status.ExitStatus())
